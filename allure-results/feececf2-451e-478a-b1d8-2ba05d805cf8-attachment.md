...
- Name: Chapter04\06_loops_Test.spec.ts >> Iteration
- Location: tests\Chapter04\06_loops_Test.spec.ts:3:5
...
```ts
  ...
  10 |   const products=await page.$$(".inventory_item_name")
  11 |   for(const product of products){
  12 |     const text=await product.textContent();
> 13 |      console.log(`Text from the 2nd loop is:"${text1}`);
     |                                                ^ ReferenceError: text1 is not defined
  14 |   }
  15 | 
  16 |   for(let i=0;i<products.length;i++){
  17 |     const text2=await products[i].textContent();
  18 |      console.log(`Text from the 2nd loop is:"${text2}`);
  19 |   }
  20 | 
  21 | });
```